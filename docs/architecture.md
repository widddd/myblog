...
- Next 16 使用 `main/src/proxy.ts`（不是已弃用的 `middleware.ts`）：统一安全头（`object-src 'none'`；`/admin` 与认证 API `Cache-Control: private, no-store`；HTTPS 时 HSTS）、所有非安全方法的 CSRF 校验、`/admin` cookie 乐观拦截、登录与创建站点 5 次/15 分/IP 限速。无管理员时放行 `/admin/setup` 与 `POST /api/auth/setup`；setup 服务端按主机半钥状态选择完整创建或仅补建管理员。
- 无管理员时根 layout 把前台与 `/admin` 转到 `/admin/setup`；创建页根据主机半钥是否存在显示完整初始化或仅重建管理员的表单。已有管理员则创建页 409 回登录；已有主机半钥时网页只创建管理员，不改站点配置或备份口令。
...
- `GET /api/auth/csrf` 创建匿名 session 并签发 double-submit token；登录成功后轮换 session 内 CSRF secret，登录响应返回新 token，并**清掉该 IP 的登录限速记账**（成功的登录不占暴力破解预算，策略见 `lib/auth/login-limit.ts`）。
- 登录/退出是鉴权边界：客户端成功后走 `window.location.replace()` **文档级跳转**，不复用软导航（避免段缓存/bfcache 里的旧载荷，也省掉 `replace + refresh` 的两次 RSC 往返）。落地地址来自 `?next=`，先过 `adminNextPath()` 白名单校验。
...
## 设计体系（theme-hao/Heo 风格，token 落地于 globals.css；后台样式独立于 admin.css）
...
- 动效：前台 slide-in / Banner / 导航进度条走 CSS，禁止重量级动画库。后台切页、弹窗、折叠只用 `motion` 的 `LazyMotion` + `domAnimation`，时长 ≤280ms，import 仅限 `src/components/admin/`（P-044 / P-071）
...
- 数据清理：更新页「危险操作」只接受 `data` / `admin` 两个范围（可同时选）；POST 先绑定当前管理员并发放一次性令牌，服务端 `executeAt` 至少晚 15 秒，客户端用 `serverNow` 校准红色倒计时，DELETE 可在执行前撤销。执行前锁住包含 `data` 的备份创建；执行时经 StorageDriver 清本地与配置 COS 桶对象，再清备份/更新暂存，最后用 Prisma 事务删除内容表；`Setting`、首页模块与主机半钥保留。两项同选后销毁 session，回到创建管理员流程。详见 [data-clear-spec.md](data-clear-spec.md)。
- 后台写文章：左侧栏目竖栏 + 可收起设置分组，右侧编辑器占大部分面积；折叠用 motion 高度动画。设置分组收起后只留标题行，左栏 Grid 不得 stretch 把收起块撑高（P-063）。分类/标签可在「分类与标签」分组当场 POST 新建并选用。收起按钮在各自栏目右上角，用 chevron 图标（`.admin-pane-toggle`），不要用「收起栏目 / 收起设置」文字。桌面后台锁视口高度，各栏 `overflow` + `overscroll-behavior: contain` 独立滚动，禁止整页带着其它栏一起滚。手机端（≤768px）设置改走底部 sheet；sheet 右上角箭头、背板与 Escape 都必须关 sheet，禁止复用桌面「收起左栏」状态（P-073）。编辑器占满，底栏固定保存操作；窄屏工具栏分两行全部露出，不要裁切。
- 后台壳：`AdminWorkspace` = 桌面左侧栏 + `.admin-main`（非编辑页顶栏当前页标题 21px + 你好用户名 15px，内容区 `.admin-stage`）。编辑页（写文章 / 首页画布 / 模块编辑）无顶栏、stage 铺满（P-033）。切页用 `AnimatePresence`（出 160ms / 入 220ms，入场位移 14px），顶栏标题只做透明度渐变。动效与字号只走 `--admin-*`：`--admin-fast/mid/slow` 为 160/220/280ms，`--admin-ease` 为 `cubic-bezier(0.16, 1, 0.3, 1)`；标题 21px、分组 17px、正文 15px。危险提示用 `.admin-danger`（`#ff3b30` 加粗）。列表/统计卡可用 `.admin-stagger` 逐项入场（460ms，唯一允许超 280ms）；`.admin-card--hover` 与统计卡允许 `translateY(-3px)`，普通 `.admin-card` 仍不加位移。禁止再给切页/折叠写超调弹簧（P-044）；弹簧只给弹窗 pop。手机端隐藏前台导航和页脚，改用固定顶栏 + 底部 5 项 tab bar（仪表盘/文章/瞬间/评论/更多）；列表同一套 DOM 桌面表格、手机卡片（P-072）。后台样式只进 `src/app/admin/admin.css`，token 挂 `:root`（P-076），**主色不固定**：6 套预设可切换（默认石墨），色值集中在 `src/lib/admin/accents.ts`，由 `src/app/admin/layout.tsx` 服务端注入 `:root:root` 的 `--admin-accent*` 锚点（浅色/暗色各一组），派生色阶与几何仍在 admin.css；不得重定义前台 `.heo-button` / `.form-field` / `.heo-card`（P-070）。按钮用 `.admin-btn`（桌面 36px 高，宽度由内容决定，禁止被 grid stretch 拉成细长条）。
