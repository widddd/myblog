...
- 投产时站点名称、管理员用户名、密码和备份口令走创建站点页或 `pnpm setup` 手动输入；已有主机半钥且管理员被清空时，网页/CLI 只补建管理员，不重新输入或替换备份口令。代码里不出现默认账号、默认口令或默认站名 MyBlog
...
- 无管理员且无主机半钥时走完整创建页或 `pnpm setup`；已有主机半钥时走创建页的“重建管理员”或 `pnpm setup` 补建账号，不能靠代码里的默认账号进后台
