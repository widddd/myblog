...
type CommentSectionProps = {
  ...
  total: number;
  pageSize?: number;
  /** All public comment threads stay collapsed until the visitor opens them. */
  collapsible?: boolean;
};
    ...
    >
      {collapsible ? (
        <button
          ...
          type="button"
        >
          <span className="comment-section__title">{title}</span>
          <span aria-hidden="true" className="comment-section__mark" />
        </button>
      ) : (
        <h2 className="comment-section__title">{title}</h2>
      )}
      {open ? (
        <div id={collapsible ? panelId : undefined}>
          {comments.length === 0 ? (
