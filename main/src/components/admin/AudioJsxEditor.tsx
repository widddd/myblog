...
export function AudioJsxEditor({ mdastNode }: JsxEditorProps) {
  ...
  async function replaceWith(file: File | undefined) {
    ...
    setBusy(true);
    try {
      const uploaded = await uploadAdminFile(file, "audio", undefined, {
        defer: true,
      });
      applySrc(uploaded.original.url);
      setLinkMode(false);
    } catch (error) {
      window.alert(error instanceof Error ? error.message : "音频更换失败");
    } finally {
      setBusy(false);
      if (inputRef.current) {
  ...
  return (
    <div className="admin-video-block">
      ...
      {playable ? (
        <audio className="post-audio" controls preload="metadata" src={src} />
      ) : (
        <p className="admin-video-missing">
          {src ? "音频地址无效，请更换文件或外链" : "尚未选择音频"}
        </p>
      )}
