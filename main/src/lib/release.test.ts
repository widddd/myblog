...
test("current release is 0.1.0", () => {
  ...
  assert.equal(APP_RELEASE_LABEL, "0.1.0");
});

test("backupReleaseLabel maps alpha packages to the public mark", () => {
  assert.equal(backupReleaseLabel("alpha", null), "0.1.0");
  assert.equal(backupReleaseLabel("alpha", ""), "0.1.0");
