...
- **前台**：主页（可在后台自由排布的模块化格点：Banner + 欢迎 + 瞬间 + 文章卡 + 侧栏 widget；推荐文章默认关闭）、文章列表/分类/标签/归档、瞬间（瀑布流/朋友圈宫格/点赞）、留言板、⌘K 搜索（密码文仅标题）。Banner 底图完全加载后再播出场动画；加载超过 2 秒才用与后台上传相同的顶栏进度条，满格后自动关掉；文章卡悬停只扫一道光泽，移开不回放；**只有真正没封面的文章**（文章设置里选「封面图」又没选图）才不留渐变占位，改成窄窄一条：只显示标题和文章开头一小段，开头那行从左到右渐渐模糊变淡（没写摘要就自动取正文开头）；选了「纯色 / 混色」的文章照样是大卡，封面位画作者选的色块；**两种卡片的右下角都有时间**，只给相对时间（最近 / 几天前 / 几个月前），无封面细条卡也一样
- **首页模块化**：后台「首页管理」可强制模拟电脑/手机画框，两套模块大小独立配置（其它文案配置共用）。模块盒子先定尺寸，内部按容器重排；宽屏两侧留白。拖动时蓝框标出落点；可改宽、改高、重置当前视口大小。可统一调模块玻璃底和首页背景的不透明度。「欢迎光临」里漂浮色块的文字可在首页管理里按行填写。「模块管理」可新建自己的模块，往里放现成积木或直接写 HTML / CSS / JS。首页底部版本信息栏中间可显示站点运行时间（精确到秒），开始时间在「设置」里填，空着则不显示
- **文章能力**：安全 MDX、目录（h1–h6 分层，点击平滑滚动到对应标题；阅读页侧栏默认只显示目录，点右侧按钮展开其它卡片）、双主题代码高亮、Video、图片灯箱（滚轮/双指放大，同一页再开走缓存）、阅读进度、浏览量 60 秒去重，2 小时签名 cookie 的密码文章；详情顶 Banner 可选封面/纯色/混色；后台可导入 Markdown。**文章页的时间**：三天以内显示「最近 / X 天前」，超过三天改成精确到秒的日期；文章发布后又被改动过，会在同一排用同一字号标出「已修改 + 修改时间」（写文章页设置栏可关掉这个显示）。**作者**：标题下面跟日期同一排、同样的灰字显示作者（笔名）
...
- **后台**：`/admin` 登录后可写文章（mdx-editor 可视化编辑 / 预览 / 导入 md；手机上点「设置」从底部打开；分类与标签可在写文章时当场新建并选用）、排首页布局、建自定义模块、管理瞬间/评论/媒体库/备份/程序更新/站点设置。侧栏按「概览 / 内容 / 资源 / 系统」分 4 组；**管理员登录后，前台导航栏右侧会出现「外观」入口**（图标紧邻主题切换，访客看不到它），点它进后台并自动展开**右上角的外观弹窗**（从外侧划入）；弹窗可切换 6 套配色（石墨 / 天青 / 藏青 / 松绿 / 绛红 / 赭石，浅色与暗色各一套锚点，换肤无需刷新），并逐张开关概览卡片——关掉的卡片不占位、其余自动重排；每张卡片右上角也能单独隐藏，全部隐藏时有空态与「恢复默认」。**后台标题行不放任何按钮**，深浅主题切换用站点顶栏那一个。手机竖屏用底部 5 项导航（仪表盘/文章/瞬间/评论/更多），列表改成卡片而不是横滑表格。**文章列表点任意一行**会弹出「查看 / 编辑」二选一：查看在新标签打开前台文章页（草稿 / 定时文章还没有公开页面，这一项置灰并说明），编辑进写文章页。更新页还提供「危险操作 → 清空数据」。删除、恢复、立刻重启等危险操作用红字加粗提示。写文章时点「图片 / 视频 / 音频」弹出插入框（上传、从媒体库导入或外链），按住顶部「拖动」换位；外链视频下方标域名；文章可勾选「首页推荐」、可关掉文章页的「已修改」显示；**作者**：写文章页的「作者」框默认带出账号里的笔名，下拉或点标签即可选已登记的笔名，也能直接打字自定义、当场新建笔名（像分类/标签那样可以攒多个）；「设置 → 登录账号」里改默认笔名，文章自己没填作者时就显示它；保存并发布会等图片压缩、生成缩略图和上云全部完成；瞬间多张为方图并可拖动排序，单张按原比例显示
...
浏览器打开 http://localhost:3000 ，未创建过管理员且没有主机半钥时会进入 **创建站点**（站点名称、管理员、备份口令）；若清空管理员后主机半钥仍在，则进入 **重建管理员**，只需设置用户名和密码，站点配置与备份口令保持不变。无界面时在 `main/` 下执行 `pnpm setup`。没有代码内默认账号，也没有默认站名 MyBlog。

生产不要跑 `pnpm db:seed`。开发若要演示文章：`pnpm db:seed`（幂等，已有文章时不会重复灌入）。清空内容或管理员请在后台更新页使用危险操作，不要直接删除正在使用的 `blog.db`。`pnpm test` / `pnpm dev` / `pnpm start` / pm2 都走 `scripts/boot.cjs`。
...
cd main
pnpm pack:update
# 或按标签：pnpm pack:update --git v0.1.1
```
